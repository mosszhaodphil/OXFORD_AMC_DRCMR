...
delta_TI=0.6
bolus_low=0.4
bolus_duration=0.6 # Estimated global bolus duration
...
# Tissue only inference in spatial VB
  # Do it in two steps: 1 inference CBF and arrival time; 2 inference CBF, arrival time, and bolus duration
  # Step 1
  $fabber --data=data_diff_tissue --data-order=singlefile --output=full_tissue_only_fixed_T1_step1 -@ fabber_tissue_options_step_1.txt
  # Update MVN
  ...
  # Step 2
  $fabber --data=data_diff_tissue --data-order=singlefile --output=full_tissue_only_fixed_T1_step2 -@ fabber_tissue_options_step_2.txt --continue-from-mvn=full_tissue_only_fixed_T1_step1/finalMVN2
  # Calibration
